...
router.post("/backups/generate", async (req, res) => {
  try {
    ...
    const dbName = dbConfig.split('/').pop().split('?')[0];
    const sqlFilePath = path.join(backupDir, `${backupName}.sql`);
    
    // Create a database backup file with metadata
    const backupContent = `-- Database Backup Generated on ${new Date().toISOString()}
-- Database: ${dbName}
...
-- Database URL: ${dbConfig.replace(/\/\/[^:]+:[^@]+@/, '//***:***@')}

-- Note: This is a metadata backup file. The actual database backup
-- should be performed using your database management tools.
-- For MySQL: mysqldump -u [username] -p [database_name] > backup.sql
-- For restoration: mysql -u [username] -p [database_name] < backup.sql

-- Current Database Tables (from Prisma schema):
-- - assignments
-- - student_assignments_cust
-- - materials
-- - centers
-- - Feedback
-- - feedback_type
